...
contract Bridge is Ownable, ReentrancyGuard, Pausable {
    ...
    function getTokenMappings(string calldata symbol) external view returns (TokenMapping[] memory) {
        // First, count how many active mappings exist
        uint256 count = 0;
        uint256[] memory foundChainIds = new uint256[](100); // Reasonable limit
        
        // Check specific known chains first
        uint256[] memory knownChains = new uint256[](10);
        knownChains[0] = 133717; // HYPERION_CHAIN_ID
        knownChains[1] = 59902;  // METIS_SEPOLIA_CHAIN_ID
        // Add other known chain IDs as needed
        
        // Check known chains
        for (uint256 i = 0; i < knownChains.length && knownChains[i] != 0; i++) {
            uint256 chainId = knownChains[i];
            if (supportedChains[chainId] && activeTokens[chainId][symbol] && tokenAddresses[symbol][chainId] != address(0)) {
                foundChainIds[count] = chainId;
                count++;
            }
        }
        ...
        for (uint256 i = 0; i < count; i++) {
            uint256 chainId = foundChainIds[i];
